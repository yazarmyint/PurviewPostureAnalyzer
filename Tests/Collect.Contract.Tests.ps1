    ...
    function Get-PpaRichStubMap {
        ...
        return @{
            ...
            'Get-DlpComplianceRule' = @{ Status = 'Ok'; Data = @(
                [pscustomobject]@{
                    Name = 'r-hipaa'; ParentPolicyName = 'HIPAA Policy'; Policy = $polGuid; Disabled = $false
            ...
            'Get-RetentionComplianceRule' = @{ Status = 'Ok'; Data = @(
                [pscustomobject]@{ Name = 'HR-Retain-7y'; Policy = $polGuid; ParentPolicyName = 'HR 7yr'; ContentMatchQuery = ''; ContentContainsSensitiveInformation = @() }) }
            ...
            'Get-LabelPolicy' = @{ Status = 'Ok'; Data = @(
                [pscustomobject]@{ Name = 'Global Policy'; Labels = @('Confidential'); Enabled = $true; ExchangeLocation = @('All'); ModernGroupLocation = @() }) }
            'Get-AutoSensitivityLabelPolicy' = @{ Status = 'Ok'; Data = @(
                [pscustomobject]@{
                    Name = 'Auto PHI'; Mode = 'TestWithoutNotifications'
            ...
            'Get-InsiderRiskPolicy' = @{ Status = 'Ok'; Data = @(
                ...
                [pscustomobject]@{ Name = 'Data theft'; InsiderRiskScenario = 'DataTheft'; Workload = @('Exchange', 'Teams'); WhenCreatedUTC = [datetime]::new(2026, 3, 15, 8, 0, 0, [System.DateTimeKind]::Utc) }) }
            ...
            'Get-ComplianceCase' = @{ Status = 'Ok'; Data = @(
                [pscustomobject]@{ Name = 'Case-1'; Status = 'Active' }) }
            'Get-SupervisoryReviewPolicyV2' = @{ Status = 'Ok'; Data = @(
                [pscustomobject]@{ Name = 'Copilot interactions'; Enabled = $true }) }
            ...
            'Get-AppRetentionCompliancePolicy' = @{ Status = 'Ok'; Data = @(
                [pscustomobject]@{ Name = 'Copilot retention'; Enabled = $true; Applications = @('User:M365Copilot') }) }
        }
    }
}
